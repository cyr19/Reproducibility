...
class BERTScorer:
    ...
    def __init__(
        ...
        self._stopwords_list = self._tokenizer.convert_tokens_to_ids(stopwords_list)
    ...
    def score(self, cands, refs, idf_sents=[], verbose=False, return_hash=False):
        """
        Args:
        ...
        if self.idf:
            ...
            if self._idf_dict is None:
                idf_dict = get_idf_dict(idf_sents, self._tokenizer, self.nthreads)
            else:
                #print('idf here')
                assert self._idf_dict, "IDF weights are not computed"
                idf_dict = self._idf_dict
