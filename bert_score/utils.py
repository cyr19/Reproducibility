...
model2layers = {
    ...
    'dbmdz/bert-base-turkish-cased': 10, # WMT18_BERT seg en-tr 0.5522827687776142
    'dbmdz/distilbert-base-turkish-cased': 4, # WMT18_BERT seg en-tr 0.4742268041237113
...
def collate_idf(arr, tokenizer, idf_dict, device="cuda:0"):
    ...
    #print(arr)
    return padded, padded_idf, lens, mask, arr
...
def bert_cos_score_idf(
    model, refs, hyps, tokenizer, idf_dict, stopwords, verbose=False, batch_size=64, device="cuda:0", all_layers=False,
):
    """
    Compute BERTScore.
    ...
    for batch_start in iter_range:
        ...
        for i, sen in enumerate(sen_batch):
           ...
           # print(idf.shape)
            ...
            stopwords_indices = [i for i in range(len(sent_tokens)) if sent_tokens[i] in stopwords]
            stopwords_indices.sort()
            #print(stopwords_indices)
            #print(len(stopwords_indices))
            count = 0
            for i in range(len(stopwords_indices)):
                emb=np.delete(emb, stopwords_indices[i-count],axis=0)
                idf=np.delete(idf, stopwords_indices[i-count])
                count+=1

            #print(emb.shape)

            #print(idf.shape)
            stats_dict[sen] = (emb, idf) #包含sent对应的emb和idf
    ...
    with torch.no_grad():
        for batch_start in iter_range:
            ...
            ref_stats = pad_batch_stats(batch_refs, stats_dict, device) #ref_embedding
